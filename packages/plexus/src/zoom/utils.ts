...
export function getZoomStyle(transform: ZoomTransform | null | undefined): React.CSSProperties {
  if (transform == null) {
...
export function getZoomAttr(transform: ZoomTransform | null | undefined) {
  if (!transform) {
    return undefined;
  }
  const { x, y, k } = transform;
